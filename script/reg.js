...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const submit = () => {
        let sendValue = {}
        sendValue['name'] = formReq[0].value;
    ...
    const validateInput = (input) => {
        switch (input.name) {
            case ("username"):
                if ((input.value.split(' ').length) !== 1|| (input.value.split(' ').length) === 0) {
            ...
            case ("password"):
                if (reqPass.value !== pass.value || reqPass.value !== "") {
                    pass.nextElementSibling.textContent = "Не совпадают";
                    reqPass.nextElementSibling.textContent = "Не совпадают";
                    errordata = false;
                } else {
    ...
    function formValidate(form) {
        for (let index = 0; index < formReq.length; index++) {
            ...
            if (!input.classList.contains('form-check') && input.tagName != "BUTTON") {
                ...
                if (input.value === '') {
                    input.nextElementSibling.textContent = "Введите данные";
                    error = false;
                }
                else {
                    input.nextElementSibling.textContent = "";
